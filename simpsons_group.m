function I = simpsons_group(f,a,b,n)

%http://uk.mathworks.com/matlabcentral/fileexchange/28726-simpson-s-rule-integration/content/simpsons.m
...
for i = 1: length(b)
    ...
    I= h/3.*(f(xi(:,1))+2*sum(f(xi(:,3:2:end-2)),2)+4*sum(f(xi(:,2:2:end)),2)+f(xi(:,end)));
%end
end